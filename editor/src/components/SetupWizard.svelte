<script lang="ts">
  ...
  import { setupsForKemper, kemperTargets, readKemperSetup, saveKemperSetup, faq, readSetup, saveSetup, piInstallCommand, type SetupId } from "../lib/setup-guide";
  ...
  let kemper = $state(readKemperSetup());
  const setups = $derived(setupsForKemper(kemper));
  const targetLabel = $derived(kemperTargets[kemper.target]);
  $effect(() => {
    if (kemper.target === "player") kemper.connection = "usb";
    saveKemperSetup(kemper);
  });
...
{#snippet content()}
  ...
  <main>
    {#if step === 0}
      ...
      <div class="fields">
        <label>Kemper model<select bind:value={kemper.target}>{#each Object.entries(kemperTargets) as [id,label]}<option value={id}>{label}</option>{/each}</select></label>
        {#if kemper.target !== "player"}
          <label>MIDI connection<select bind:value={kemper.connection}><option value="usb">USB</option><option value="din">Two MIDI DIN cables</option></select></label>
          <label>Hardware generation<select bind:value={kemper.generation}><option>MK1</option><option>MK2</option></select></label>
          <label>Mode set on the Kemper<select bind:value={kemper.mode}><option value="performance">Performance</option><option value="browse">Browse</option></select></label>
        {/if}
      </div>
      {#if kemper.target !== "player"}<p class="note">Head, Rack and Stage share the experimental PROFILER plugin. Select generation and mode manually; hardware validation is pending. MK2 Fixed FX also requires compatible Kemper OS.</p>{/if}
    ...
    {:else if step === 2}
      ...
      {:else}<p>You do not need software on Android or Raspberry Pi. Once you have saved the Captain profile, disconnect it from your computer and follow the connection diagram for your {targetLabel}.</p>{/if}
    {:else if step === 3 && setup}
      <p><strong>{setup.title}</strong> - {setup.outcome}</p><SetupDiagram id={setup.id} target={kemper.target} connection={kemper.connection} />
