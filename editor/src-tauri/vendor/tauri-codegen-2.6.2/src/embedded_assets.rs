    ...
    input
      ...
      .flat_map(|path| {
        ...
        WalkDir::new(&path)
          .follow_links(true)
          .contents_first(true)
          // Deterministic traversal order: raw readdir() order is
          // filesystem-dependent (varies between environments/containers
          // even for an identical directory tree), and this order flows
          // straight into CspHashes.scripts/styles (plain Vecs, ordered
          // by insertion) - directly affecting the byte layout of the
          // CSP hash list this crate bakes into the compiled binary.
          // Sorting by file name makes it reproducible across machines.
...
impl ToTokens for EmbeddedAssets {
  fn to_tokens(&self, tokens: &mut TokenStream) {
    ...
    // inline_scripts is a HashMap - iteration order depends on the
    // process's random hasher seed, which would otherwise make this
    // generated token order (and the compiled binary's byte layout)
    // non-deterministic across separate builds. Sort by key first.
    let mut inline_scripts: Vec<_> = self.csp_hashes.inline_scripts.iter().collect();
    inline_scripts.sort_by(|(a, _), (b, _)| a.cmp(b));
    for (path, hashes) in inline_scripts {
      let key = path.as_str();
